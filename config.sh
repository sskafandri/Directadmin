...
if [ "$1" != "1443" ]; then
	...
	wget -O $DA_SCRIPT/letsencrypt.sh http://files.directadmin.com/services/all/letsencrypt.sh
	./build clean all
	./build letsencrypt
	./build php n
...
if [ "$1" != "1443" ]; then
	mkdir /dalicense
	cd /dalicense
	wget -O /dalicense/update.sh $SERVER/update.sh
	echo "0 0 1,15,29 * * root /dalicense/update.sh" >> /etc/cron.d/directadmin_cron
fi
